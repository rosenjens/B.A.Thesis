...
When it comes to giving instructions to the computer, it has now for several decades been the standard that the interaction is done through the use of mouse and keyboard\cite{05489218}. They are well suited for the window icon menu pointer system. For humans to interact and navigate with a three dimensional environment, a mouse and keyboard can be hard to use\cite{?}. A more appropriate way would be to use a three dimensional user interface. 
%what is it?
This would allow the user to perform the input in three dimensional space by using e.g. direct hand motions.
